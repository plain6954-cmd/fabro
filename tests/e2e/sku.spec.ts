...
test('SKU page supports add, search, edit and delete', async ({ page }) => {
  ...
  await page.locator('button[name="add_sku"]').click({ noWaitAfter: true });
  ...
  await page.locator('tr', { hasText: sample.skuCode }).locator('.action-btn.edit').click();
  await page.locator('textarea[name="description"]').fill('Updated Playwright SKU');
  await page.getByRole('button', { name: /Update SKU/i }).click({ noWaitAfter: true });
  await expect(page).toHaveURL(/\/add-sku\/$/);

  page.once('dialog', (dialog) => dialog.accept());
  await page.locator('tr', { hasText: sample.skuCode }).locator('.action-btn.delete').click();
